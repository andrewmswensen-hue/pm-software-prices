...
can be served by GitHub Pages from the /docs folder. The page includes:
  ...
  - Hall of Shame (gated vendors)
  - Filterable, sortable table of all vendors
...
def render():
    ...
    # Sort shame by category then name
    shamed.sort(key=lambda v: (v["category"], v["name"]))

    # Categories present in the table (excluding gated, which is in Hall of Shame)
    ...
    # Render table rows (non-gated vendors)
...
<header>
  <div class="container">
    ...
    <div class="meta">
      ...
      <span class="dot">·</span>
      <span>Maintained by <strong>{esc(meta["maintainer"])}</strong></span>
    </div>
...
<main class="container">
  ...
  <section id="hall-of-shame">
    <h2>🔒 Hall of Shame</h2>
    <p class="section-desc">These vendors expect property managers to sit through a sales call before learning the price. Most of their competitors don't.</p>
...
<footer>
  <div class="container">
    ...
    <p>Every Monday, an automated job visits each public pricing page, snapshots it, and compares it to the previous week. Any change — a new tier, a price increase, a removed plan — is committed to this repo. The full pricing history lives in the git log.</p>
...
'''
    ...
    print(f"docs/index.html generated — {len(vendors)} vendors, {gated} in Hall of Shame")
